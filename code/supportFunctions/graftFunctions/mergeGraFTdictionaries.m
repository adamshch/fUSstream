...
%         idx   = (S(:,combs(ll,1))>0)&(S(:,combs(ll,2))>0);                 % ... Find the indices that are shared
%         TMPv1 = S(idx,combs(ll,1))';                                       % ... Get the pixel values for the first profile
%         TMPv2 = S(idx,combs(ll,2))';                                       % ... Get the pixel values for the second profile
%         goodI = (TMPv1>0.01*max(TMPv1(:)))&(TMPv2>0.01*max(TMPv2(:)));     % ... Find the locations where both profiles are significant
%         TMPv1 = TMPv1(goodI);                                              % ... Get the significant pixel values for the first profile
%         TMPv2 = TMPv2(goodI);                                              % ... Get the significant pixel values for the second profile
...
%         aRatio(combs(ll,1),combs(ll,2)) = nanmedian(TMPv1./TMPv2);         % ... aRatio is the median ratio between significant pixels
%         aRatio(combs(ll,2),combs(ll,1)) = nanmedian(TMPv2./TMPv1);         % ... This matrix is NOT symmetric as Aij = 1/Aji
    end
end
...
ratioOut = zeros(size(numOL,1),1);

%numOL = numOL.*(ones(size(numOL))-diag(size(numOL)));                      % Remove the diagonal for convenience
numOL(1:(size(numOL,1)+1):end) = 0;
numOL(isnan(numOL)) = 0;                                                   % Just in case NaNs exist... those should be zero

% sumRatios = nansum(aRatio,1);
...
%sumNaN  = isnan(sum(aRatio,1));                                            % 
%minOL   = min(numOL,[],1);
%minOL   = minOL.*(~sumNaN);
%bestRow = find(minOL==max(minOL(:)));
%aRatio  = aRatio(bestRow(1),:);
...
aa = MASK.*(sum(A_subset*diag(aRatio),2));
nA = vec(sum(A_subset>0,1)./sum(sum(A_subset>0)));
