function [A,C,nr,merged_ROIs] = merge_components3(A,C,options,normalizeSpatial,merged_ROIs)
...
aRatio  = getSpatialWeights(A_subset);
...
function aRatio = getSpatialWeights(S)
...
for ll = 1:size(combs,1)
    ...
    if TMP ==0
        ...
        idx   = (S(:,combs(ll,1))>0)&(S(:,combs(ll,2))>0);
        TMPv1 = S(idx,combs(ll,1))';
        TMPv2 = S(idx,combs(ll,2))';
        goodI = (TMPv1>0.01*max(TMPv1(:)))&(TMPv2>0.01*max(TMPv2(:)));
        TMPv1 = TMPv1(goodI);
        TMPv2 = TMPv2(goodI);
        aRatio(combs(ll,1),combs(ll,2)) = median(TMPv1./TMPv2);
        aRatio(combs(ll,2),combs(ll,1)) = median(TMPv2./TMPv1);
...
function aRatio = pickBestRatios(aRatio,numOL)
...
aa = MASK.*(mean(A_subset*diag(aRatio),2));
...
function cc = mergeSingleTempotal(C_subset,nA);

cc = vec(sum(diag(nA)*C_subset));
