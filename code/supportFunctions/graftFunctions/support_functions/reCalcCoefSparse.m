...
if nargin > 4; nonneg = varargin{1};
else           nonneg = false;
end

if nargin > 5; solverOpt = varargin{2};
	...
	fprintf('Nonnegative LASSO chosen.\n')
        ...
        parfor ll = 1:nPix                                                 % Loop through all the pixels
            ...
            S_out = cast(quadprog(DtD, f, [], [], [], [],...
                      zeros(nCoef,1), Inf*ones(nCoef,1), S(ll,:), qp_opts), classS); % Use quadratic programming to solve the non-negative LASSO
            max_val = max(S_out);
            S_out(S_out<0.1*max_val) = 0;
            S(ll,:) = S_out;
        end
...
elseif strcmp(lower(solverOpt),'omp')                                      % Solve using OMP
    if nonneg
        ...
        parfor ll = 1:nPix                                                 % Loop through all the pixels
	    S(ll,:) = cast(omp(D, vec(data(ll,:)), DtD), classS);        % Run OMP
        end
    end		    
end
